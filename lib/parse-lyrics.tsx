...
export function parseLyrics(lyricsText: string, audioAnalysis?: { beats: number[]; duration: number }): LyricPair[] {
  ...
  if (audioAnalysis && audioAnalysis.beats.length > 0) {
    ...
    pairs.forEach((pair, i) => {
      pair.startTime = i * timePerPair
      pair.endTime = (i + 1) * timePerPair
    })
  }
...
const parseTimedLyrics = (lyrics: string): TimedLine[] => {
  ...
  for (const line of lines) {
    ...
    if (timeMatch) {
      ...
      const time = Number.parseFloat(minutes) * 60 + Number.parseFloat(seconds)
      const text = trimmedLine.replace(timeMatch[0], "").trim()
      timedLines.push({ line: text, time })
    }
  }
...
const assignTimingWithSectionsOld = (sections: Section[], timedLines: TimedLine[]): Section[] => {
  let timedLineIndex = 0

  for (let i = 0; i < sections.length - 1; i++) {
    const section = sections[i]
    section.startTime = timedLines[timedLineIndex]?.time || 0

    let linesInSection = 0
    while (timedLineIndex < timedLines.length && section.lines.includes(timedLines[timedLineIndex].line)) {
      timedLineIndex++
      linesInSection++
    }

    sections[i].endTime = timedLines[timedLineIndex]?.time
  }

  sections[sections.length - 1].startTime = timedLines[timedLineIndex]?.time || 0
  return sections
}

export { parseTimedLyrics, assignTimingWithSectionsOld as assignTimingWithSections }
